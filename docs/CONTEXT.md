...
- `calc/engine.py` (M1-real ✅): Full shunting-yard pipeline (tokenize → RPN → evaluate). ASCII operators `+ - * /`, parentheses, decimals (including `.5` leading-dot style). Correct precedence (`*`/`/` = 2, `+`/`-` = 1, left-associative). Division by zero / malformed / empty / non-str input → `"Error"`. Never raises. Result formatting: whole numbers lose `.0`, fractional up to 15 significant digits.
...
- Tests (6 files): `tests/test_engine.py` (111 tests — comprehensive shunting-yard coverage), `tests/test_controller.py`, `tests/test_view.py`, `tests/test_view_takefocus.py`, `tests/test_view_wiring.py`, `tests/test_package_wiring.py`. No test may import `tkinter` directly or create a live display.
- **Pass 1 (STUBS) complete. Pass 2 (REAL): M1-real done (248 tests green). Next: M2-real (controller state machine).**
